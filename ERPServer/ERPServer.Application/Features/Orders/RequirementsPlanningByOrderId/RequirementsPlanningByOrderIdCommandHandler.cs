...
{
    public sealed class RequirementsPlanningByOrderIdCommandHandler(IOrderRepository orderRepository, IStockMovementRepository stockMovementRepository, IRecipeRepository recipeRepository,IUnitOfWork unitOfWork) : IRequestHandler<RequirementsPlanningByOrderIdCommand, Result<RequirementsPlanningByOrderIdResponse>>
    {
        ...
        {
            Order? order = await orderRepository.Where(p => p.Id == request.OrderId).Include(p => p.Details!)
                .ThenInclude(p => p.Product).FirstOrDefaultAsync(cancellationToken);
            if (order is null)
                return Result<RequirementsPlanningByOrderIdResponse>.Failure("Sipariş Bulunumadı");
            ...
            requirementsPlaningProducts = requirementsPlaningProducts.GroupBy(p => p.Id).Select(g => new ProductDto
            {
                Id = g.Key,
                Name = g.First().Name,
            ...
            return new RequirementsPlanningByOrderIdResponse(DateOnly.FromDateTime(DateTime.Now), order.Number + " Nolu Siparişin İhtiyaç Planlaması", requirementsPlaningProducts);
        }
    }
}
