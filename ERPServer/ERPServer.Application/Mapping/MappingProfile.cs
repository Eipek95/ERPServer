...
namespace ERPServer.Application.Mapping
{
    public sealed class MappingProfile : Profile
    {
        public MappingProfile()
        {
            ...
            CreateMap<UpdateOrderCommand, Order>().ForMember(member => member.Details, opt => opt.Ignore());//detay kısmını maplemez
            ...
            CreateMap<CreateInvoiceCommand, Invoice>()
                ...
                ForMember(member => member.Details, options => options.MapFrom(p => p.Details.Select(s => new InvoiceDetail
                {
                    Price = s.Price,
                    ProductId = s.ProductId,
                    Quantity = s.Quantity,
